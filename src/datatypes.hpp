...
private:
    struct Cell {
        ...
        T value;
        Cell() { };
        Cell(Key key, T value)
            : key(key)
            , value(value) { };
        Cell(std::tuple<Key, T> tup)
            : key(std::get<0>(tup))
            , value(std::get<1>(tup))
        {
        }
...
public:
    ...
    Heap(const Heap<Key, T, CompareFunc>& other)
        : m_cap(other.m_cap)
        , m_size(other.m_size)
    {
        m_data = new Cell[m_cap];
    ...
    Heap(Heap<Key, T, CompareFunc>&& other)
        : m_cap(other.m_cap)
        , m_size(other.m_size)
    {
        m_data = other.m_data;
...
public:
    ...
    inline std::tuple<Key, T*> search(const T& comp)
    {
        ...
        if (!found)
            return std::make_tuple<Key, T*>({}, nullptr);
...
public:
    std::tuple<K, T*> predecessor(const K& key) const
    {
        ...
        auto left_child = max_impl(node->left);
        if (left_child)
            return { left_child->key, &left_child->data };
    ...
    std::tuple<K, T*> successor(const K& key) const
    {
        ...
        if (right_child)
            return { right_child->key, &right_child->data };
...
public:
    inline UnionFind(const std::vector<T>& v)
        : UnionFind(v.data(), v.size())
    {
    }
...
class LinkedList {
    ...
    Cell* m_head {};
    Cell* m_tail {};
    inline static constexpr const std::size_t INITIAL_CAP = 16;

public:
    inline LinkedList() noexcept
    {
    }
    inline void append_back(const T& elem)
    {
        if (!m_head) {
            m_head = new Cell;
            m_tail = m_head;
            ...
            new_cell->prev = m_tail;
            m_tail = new_cell;
        }
        m_len++;
    }
    inline void append_front(const T& elem)
    {
        if (!m_head) {
            m_head = new Cell;
            m_tail = m_head;
            ...
            auto new_cell = new Cell;
            new_cell->val = elem;
            m_head->prev = new_cell;
            new_cell->next = m_head;
            m_head = new_cell;
        }
        m_len++;
    }
    inline std::pair<T*, bool> find(const T& val) const
    {
        auto* current = m_head;
    ...
    inline void remove_at(const std::size_t& idx)
    {
        auto* current = m_head;
        std::size_t i = 0;
        while (current != nullptr) {
    ...
    inline std::pair<T*, bool> operator[](const std::size_t& idx)
    {
        auto* current = m_head;
        std::size_t i = 0;
        while (current != nullptr) {
    ...
    inline std::pair<const T*, bool> operator[](const std::size_t& idx) const
    {
        auto* current = m_head;
        std::size_t i = 0;
        while (current != nullptr) {
    ...
    inline LinkedList(const LinkedList& other)
    {
        m_len = other.m_len;
        auto* current = m_head;
        while (current != nullptr) {
            append_back(current->val);
            current = current->next;
        }
    }
    inline LinkedList& operator=(const LinkedList& other)
    {
        m_len = other.m_len;
        auto* current = m_head;
        while (current != nullptr) {
    ...
    inline LinkedList(LinkedList&& other)
    {
        m_len = other.m_len;
        m_head = other.m_head;
        m_tail = other.m_tail;
    ...
    inline LinkedList& operator=(LinkedList&& other)
    {
        m_len = other.m_len;
        m_head = other.m_head;
        m_tail = other.m_tail;
    ...
    inline std::pair<T*, bool> front()
    {
        const auto guard = m_head != nullptr;
        if (guard) {
            return { &m_head->val, guard };
        }
        return { nullptr, guard };
    }
    inline std::pair<T*, bool> back()
    {
        const auto guard = m_tail != nullptr;
        if (guard) {
            return { &m_tail->val, guard };
        }
        return { nullptr, guard };
    }
    inline ~LinkedList()
